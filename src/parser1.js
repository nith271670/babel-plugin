    ...
    if (currentNode.type === 'JSXElement') {
      if (currentNode.openingElement.name.name == 'Label') {
        if (currentNode.openingElement.attributes.length > 0) {
          ...
          for (
            var i = 0;
            i < currentNode.openingElement.attributes.length;
            i++
          ) {
          ...
          if (uuid_exist) {
            ...
            for (
              ...
              if (
                currentNode.openingElement.attributes[i].name.name != 'uuid'
              ) {
                attributes_arr[
                  currentNode.openingElement.attributes[i].name.name
                ] = currentNode.openingElement.attributes[i].value.value;
              }
            }
            element = {
              [uuid]: attributes_arr,
...
export const getTree = (content, plugin) => {
  const rawAst = plugin
    ? plugin.parse(content, {
        sourceType: 'module',
        plugins: ['jsx'],
      })
